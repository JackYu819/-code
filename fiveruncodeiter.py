...
# @Last Modified time: 2017-10-30 11:17:20
...
# 得到初始权重
def give_avg_weights_array(feature_num):
    orig_w = np.array(list([1]) * feature_num)
    orig_w_nor = orig_w / orig_w.sum()
    
    return orig_w_nor


# 通过迭代法来求解权重
def get_weights_iteration(rows_without_label, original_weights, error_param = 1e-4, max_iter = 500, alpha = 0.75):
    columns_name_list = rows_without_label.columns
    # 初始状态
    error_std = max(rows_without_label.std()) - min(rows_without_label.std())
    # 初始化迭代次数
    iter_count = 0
    while (error_std >= error_param) and (iter_count <= max_iter):
        ...
        original_weights[index_id] = original_weights[index_id] * alpha
        #original_weights = np.array([abs(i) for i in original_weights])
...
if __name__ == '__main__':
    ...
    result_df = pd.DataFrame(columns = ['KNN-mean', 'My-mean','iter-mean','KNN-std','My-std', 'iter-std'])
    ...
    for data_path in file_path_list:
        ...
        for alpha_value in arg_list:
            ...
            for ii in range(50):
                ...
                original_weights = give_avg_weights_array(feature_num)
                iteration_train_df = pd.DataFrame(columns=new_train_df.columns)
                for ix, rows in train_df.groupby('label'):
                    # print(ix)
                    rows_without_label = rows.drop('label', axis=1)
                    classic_weights, iter_step = get_weights_iteration(rows_without_label, original_weights, error_param=1e-3, alpha=0.01)
                    
                    # print(classic_weights)
                    tmp_data_times_weights = rows_without_label * classic_weights
                    tmp_sum = tmp_data_times_weights.sum(axis = 1)
                    # print(tmp_sum.mean())
                    # print(tmp_sum.std())
                    classic_weights.extend([tmp_sum.mean(), tmp_sum.std(), ix])
                    iteration_train_df.loc['{name}:{iter_n}'.format(name = ix, iter_n = iter_step),:] = classic_weights
                
                del ix, result
                iter_result_list = []
                test_df_without_label = test_df.drop('label', axis = 1)
                for ix, cont in test_df_without_label.iterrows():
                    result_t = center_weights_func(iteration_train_df, cont, center_vector, alpha = alpha_v)
                    # print(result, test_df.loc[ix].label)
                    iter_result_list.append(result_t)
                iter_knn_result = accuracy_score(iter_result_list, test_df['label'])
                ...
                iter_knn.append(iter_knn_result)
                
                
            result_df.loc[alpha_v, :] = [np.mean(classic_knn), np.mean(my_knn),np.mean(iter_knn), np.std(classic_knn), np.std(my_knn), np.std(iter_knn)]
